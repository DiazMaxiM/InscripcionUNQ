...
export class ModificacionDeIncidenciaDialogoComponent implements OnInit {
	...
	crearFiltroTipoEstadoIncidencia() {
		this.filtroTipoEstadoIncidencia = this.form.controls['tipoEstadoIncidencia'].valueChanges.pipe(
			startWith(''),
	...
	armarIncidencia() {
		const { tipoIncidencia, descripcion, tipoEstadoIncidencia } = this.form.value;
