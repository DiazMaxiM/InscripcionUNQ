...
public class LoadInitialDataController {
	...
	@Autowired
	private GenericService<Matter> matterServiceImp;
	...
	public void loadData() {
		...
		matt.addCareer(tpi);
		matt.addCareer(lds);
		long intro = matterServiceImp.save(matt);

		matt = new Matter("01032", "Organización de Computadoras", 6);
		matt.addCareer(tpi);
		matt.addCareer(lds);
		matterServiceImp.save(matt);
		...
		Commission comm1 = new Commission("Intro1", matterServiceImp.get(intro), 30);
		comm1.addHours(TypeDay.FRIDAY, LocalTime.of(10, 00), 2);
		commissionServiceImp.save(comm1);

		Commission comm2 = new Commission("Intro2", matterServiceImp.get(intro), 30);
