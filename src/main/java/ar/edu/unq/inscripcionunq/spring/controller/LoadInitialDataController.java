...
public class LoadInitialDataController {
	...
	@Autowired
	private MatterService matter;

	@Autowired
	private CommissionService commission;

	@Autowired
	private AcademicOfferService academicOffer;
	...
	public void loadData() {
		...
		tpi = career.get(career.save(tpi));
		lds = career.get(career.save(lds));

		Matter matt = new Matter("00487", "Introducción a la Programación", 8);
		matt.addCareer(tpi);
		matt.addCareer(lds);
		long intro = matter.save(matt);
		...
		Commission comm1 = new Commission("Intro1", matter.get(intro), 30);
		comm1.addHours(TypeDay.FRIDAY, LocalTime.of(10, 00), 2);
		commission.save(comm1);

		Commission comm2 = new Commission("Intro2", matter.get(intro), 30);
		comm1.addHours(TypeDay.MONDAY, LocalTime.of(14, 00), 2);
		commission.save(comm2);
		...
		academicOffer.save(acc1);
		academicOffer.save(acc2);
